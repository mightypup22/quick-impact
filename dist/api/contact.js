...
module.exports = async (req, res) => {
  try {
    ...
    // Body lesen (Node runtime, nicht Edge)
    const chunks = [];
    ...
    const message = (body.message || body.anliegen || '').trim();
    const consent = toBool(body.consent);
    const hp      = (body.website || '').trim(); // Honeypot

    if (hp) return json(res, 200, { ok:true, message:'Thanks.' });
...
`;
    ...
    if (typeof fetch !== 'function') {
      return json(res, 500, { ok:false, error:'Server: fetch() nicht verfügbar (Runtime muss Node 18 sein).' });
    }

    const resp = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ from: MAIL_FROM, to: [MAIL_TO], reply_to: email, subject, html, text })
    });

    const data = await resp.json().catch(()=>null);
    if (!resp.ok) return json(res, 502, { ok:false, error:'Email-Versand fehlgeschlagen.', detail:data });

    // Für No-JS-Fallback (klassischer Form-Post): zurück zur Seite
...
// Force Node runtime (avoid Edge where Buffer/fetch semantics differ)
